...
public class Formulaire_Partie3DaoImpl implements Formulaire_Partie3Dao {
    ...
    public List<Formulaire_Partie3> GetAllFormPartie3(String date_debut, String date_fin) {

        List<Formulaire_Partie3> Liste_form_partie_3 = new ArrayList<>();

        String query = "SELECT * FROM info_sensibilisation info \n" +
                "where date(info.date_creation) >= '" + date_debut + "'\n" +
                " and date(info.date_creation) <= '" + date_fin + "'" +
                " GROUP BY id_user\n";
